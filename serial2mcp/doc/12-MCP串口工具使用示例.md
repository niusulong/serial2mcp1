...
```
**注意**: keyword模式将持续接收数据直到找到停止模式（stop_pattern）或超时。一旦找到停止模式，将立即返回结果，无需等待完整超时时间。如果未找到停止模式，则会抛出超时错误。

### 2.3 查询设备制造商信息（使用timeout模式）
```json
{
  "name": "send_data",
  "arguments": {
...
```
**注意**: timeout模式将在指定超时时间内收集所有数据，无论是否包含特定模式。
...
```
**注意**: at_command模式将在指定超时时间内收集所有数据，包括回显和响应。
...
```
**注意**: at_command模式将返回超时时间内接收到的所有数据。
...
```
**注意**: keyword模式将在超时时间内收集所有数据，然后检查是否包含"> "提示符。
