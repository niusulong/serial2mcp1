...
│       ├── main.py              # MCP服务器入口点
│       ├── server.py            # MCP服务器主类
│       ├── tools/               # 工具层 - 实现具体的MCP工具功能
│       │   ├── __init__.py
│       │   ├── base.py          # 基础工具类，提供统一的基类和通用功能
│       │   ├── connection.py    # 连接管理工具，负责串口连接和断开操作
│       │   ├── communication.py # 通信工具，负责串口数据的发送和接收操作
│       │   └── async_message.py # 异步消息处理工具，负责处理设备主动上报的消息
│       ├── driver/              # 核心驱动层 - 串口底层通信实现
│       │   ├── __init__.py
│       │   ├── serial_driver.py # 串口驱动主类
│       │   ├── reader.py        # 后台接收线程
│       │   ├── processor.py     # 数据处理器
│       │   └── connection_manager.py # 连接管理
│       ├── facade/              # 门面层 - 提供统一的MCP工具接口
│       │   ├── __init__.py
│       │   ├── tool_facade.py   # 工具门面，协调各个工具模块并提供统一的MCP接口
...
1. **接口层 (main.py)**：MCP协议处理和服务器管理
2. **门面层 (facade/)**：统一接口和工具协调
3. **工具层 (tools/)**：具体MCP工具功能实现
4. **驱动层 (driver/)**：底层串口通信实现
...
class SerialDriver:
    ...
    def __init__(self):
        ...
        self._async_queue = queue.Queue(maxsize=self.config.driver.async_buffer_size)
...
class PatternMatcher:
    ...
    def __init__(self):
        ...
        self.async_regex = [re.compile(pattern, re.IGNORECASE)
    ...
    def identify_data_type(self, data: str, context: Dict[str, Any]) -> DataType:
        ...
        for regex in self.async_regex:
            if regex.search(data):
                return DataType.ASYNC_MESSAGE
        ...
        # 默认根据模式判断
        return DataType.ASYNC_MESSAGE if context.get('idle_mode', False) else DataType.UNKNOWN
...
class DelayedAsyncMessageBuffer:
    """异步消息延迟缓冲区"""
...
class BackgroundReader(threading.Thread):
    def __init__(self, driver: 'SerialDriver'):
        ...
        self.delayed_async_buffer = DelayedAsyncMessageBuffer()
    ...
    def _handle_sync_mode_intelligent(self, packet: DataPacket, current_time: float) -> None:
        """智能同步模式处理"""
        if packet.data_type == DataType.ASYNC_MESSAGE:
            # 异步消息进入延迟缓冲区
            self.delayed_async_buffer.add(packet)
            self.stats['delayed_async_count'] += 1
        else:
            # 响应数据直接处理
            self._send_to_response_queue(packet)

            # 检查是否有延迟的异步消息需要处理
            if self.delayed_async_buffer.should_flush(current_time):
                self._flush_delayed_async()

    def _handle_idle_mode_intelligent(self, packet: DataPacket) -> None:
        """智能空闲模式处理"""
        # 空闲模式下，所有数据都作为异步消息处理
        packet.data_type = DataType.ASYNC_MESSAGE
        self.driver._async_buffer.append(packet)
    ...
    def _flush_delayed_async(self) -> None:
        """刷新延迟的异步消息"""
        delayed_packets = self.delayed_async_buffer.flush()
        ...
        for packet in delayed_packets:
            ...
            self.driver._async_queue.put(async_message)
    
    def _check_timeouts(self, current_time: float) -> None:
        """检查各种超时"""
        # 检查空闲超时
        if (self.driver._async_buffer and
            current_time - self.driver._last_received_time > self.IDLE_THRESHOLD):
            self._pack_async_buffer()

        # 检查延迟异步消息缓冲区
        if self.delayed_async_buffer.should_flush(current_time):
            self._flush_delayed_async()

    def _pack_async_buffer(self) -> None:
        """打包异步消息缓冲区数据（增强版）"""
        if not self.driver._async_buffer:
            return

        # 按时间戳排序
        sorted_packets = sorted(self.driver._async_buffer, key=lambda p: p.timestamp)
        ...
        # 清空缓冲区
        self.driver._async_buffer.clear()
    ...
    def _update_stats(self, data_type: DataType) -> None:
        ...
        elif data_type == DataType.ASYNC_MESSAGE:
            self.stats['async_packets'] += 1
        else:
...
flowchart TD
    ...
    D -->|read_async_messages| H[读取异步消息]
